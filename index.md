...
1. **Configurator** &mdash; an admin web app. Paste a URL, see every image
   on the page, draw regions on those images, attach products to each
   region (manual link or catalog pick), then publish.
2. **Runtime** &mdash; a small, sandboxed `<script>` tag the host site
   embeds once. It finds the configured images on the live page, mounts
   hotspots over the regions, and shows product tooltips on hover/tap.
